...
// SHIFT
function openShiftModal(empName, empId) {
    empIdShiftModal = empId;
    fetch(`/getShift?employeeId=${empId}`).then(res => res.json()).then(shift => {
        if (shift.success == false) {
            showDiv("noShift");
        } else {
            // inject the data
    ...
    document.getElementById("shiftModalTitle").innerText = `تعديل الشفت للموظف: ${empName}`;
